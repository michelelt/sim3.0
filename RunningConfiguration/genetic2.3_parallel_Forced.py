...
pure_elitist=int(sys.argv[1]) # if set to 1 it converges faster (exploitation), if set to 0 it explores more (exploration)
#### 
...
def NewGeneration(Parents, Children):
    ...
    if AllTimeBest!=Sorted_Solutions[0][1]:
        ...
        fbest.write("New best solution found: " +str(AllTimeBest) +'\n'  )
        not_improving=0
    else:
        not_improving+=1
...
def InitParents():
    ...
    flog.write("simulazioni parallele: " + str(len(Solutions)) +'\n')
    ...
    flog.write("w8ing %d"%len(jobs)+'\n')
    for proc in jobs:
        proc.join()
        
    print("keys: ", return_dict.keys())
    flog.write("keys: " + str(return_dict.keys())+'\n')
    for val in return_dict.values(): 
        ...
        flog.write(str(deaths)+" "+str(wwd)+" "+str(childi) +'\n')
...
def main():
    ...
    while True and not_improving<=Thresh_not_improving:
        ...
        flog.write("simulazioni parallele: "+str(len(Childs))+'\n')
        ...
        for val in return_dict.values():           
            ...
            deaths = val["PercDeath"]
            wwd = val['WeightedWalkedDistance']
            rer=val["PercRerouteEnd"]
            mean_wd=val["MeanMeterEnd"]
            recharge=val["PercRecharge"]
            print(deaths,wwd,mean_wd,rer,recharge,childi)    
